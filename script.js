...
function initializeTooltips() {
    ...
    tooltipIcons.forEach(icon => {
        icon.addEventListener('mouseenter', function() {
            this.style.animation = 'iconPulse 0.6s ease infinite';
        });
        ...
        icon.addEventListener('click', function(e) {
            ...
            createTooltip(card);
            console.log('Added tooltip-active class');

            setTimeout(() => {
                card.classList.remove('tooltip-active');
                removeAllTooltips();
                console.log('Removed tooltip-active class');
            }, 4000);
        });
    });

    const tooltipCards = document.querySelectorAll('.card2[data-tooltip]');
...
function createTooltip(card) {
    ...
    tooltip.style.cssText = `
        position: fixed;
        bottom: ${window.innerHeight - cardRect.top + 15}px;
...
function getWeatherData(city, unit, hourlyorWeek) {
    if (!loadingSpinner.classList.contains("hidden")) {} else {
        showLoading();
    }
    ...
    fetch(
        ...
        .catch((err) => {
            hideLoading();
            hideCardSpinners();
            showNotification("City not found in our database. Please try another location.", "error");
        });
}
...
function measureUvIndex(uvIndex) {
    if (uvIndex <= 2) {
        ...
        uvText.style.color = "#4CAF50";
    } else if (uvIndex <= 5) {
        uvText.innerText = "Moderate";
        uvText.style.color = "#FF9800";
    } else if (uvIndex <= 7) {
        uvText.innerText = "High";
        uvText.style.color = "#FF5722";
    } else if (uvIndex <= 10) {
        uvText.innerText = "Very High";
        uvText.style.color = "#E91E63";
    } else {
        uvText.innerText = "Extreme";
        uvText.style.color = "#9C27B0";
    }
}
...
function updateHumidityStatus(humidity) {
    if (humidity <= 30) {
        ...
        humidityStatus.style.color = "#FF9800";
    } else if (humidity <= 60) {
        humidityStatus.innerText = "Comfortable";
        humidityStatus.style.color = "#4CAF50";
    } else {
        humidityStatus.innerText = "Humid";
        humidityStatus.style.color = "#2196F3";
    }
}
...
function updateVisibiltyStatus(visibility) {
    if (visibility <= 1) {
        ...
        visibilityStatus.style.color = "#2196F3";
    }
}
...
function updateAirQualityStatus(airquality) {
    if (airquality <= 50) {
        ...
        airQualityStatus.style.color = "#4CAF50";
    } else if (airquality <= 100) {
        airQualityStatus.innerText = "Moderate";
        airQualityStatus.style.color = "#FFEB3B";
    } else if (airquality <= 150) {
        airQualityStatus.innerText = "Unhealthy (Sensitive)";
        airQualityStatus.style.color = "#FF9800";
    } else if (airquality <= 200) {
        airQualityStatus.innerText = "Unhealthy";
        airQualityStatus.style.color = "#FF5722";
    } else if (airquality <= 300) {
        airQualityStatus.innerText = "Very Unhealthy";
        airQualityStatus.style.color = "#E91E63";
    } else {
        airQualityStatus.innerText = "Hazardous";
        airQualityStatus.style.color = "#9C27B0";
    }
}
...
fahrenheitBtn.addEventListener("click", () => {
    addButtonClickEffect(fahrenheitBtn);
    changeUnit("f");
});

celciusBtn.addEventListener("click", () => {
    addButtonClickEffect(celciusBtn);
    changeUnit("c");
});
...
hourlyBtn.addEventListener("click", () => {
    addButtonClickEffect(hourlyBtn);
    changeTimeSpan("hourly");
});
weekBtn.addEventListener("click", () => {
    addButtonClickEffect(weekBtn);
    changeTimeSpan("week");
});
...
function changeTimeSpan(unit) {
    if (hourlyorWeek !== unit) {
        ...
        if (unit === "hourly") {
            ...
            showNotification("Switched to hourly forecast", "info");
        } else {
            hourlyBtn.classList.remove("active");
            weekBtn.classList.add("active");
            showNotification("Switched to weekly forecast", "info");
        }
        getWeatherData(currentCity, currentUnit, hourlyorWeek);
    }
}
